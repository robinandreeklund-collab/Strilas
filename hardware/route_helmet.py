#!/usr/bin/env python3
"""STRILAS — route hjälm-noden (Ø100, 4-lager): DSN → power-klass (0,4 mm) → freeroute (loop) →
ses_apply → kopparplan (In1=GND, In2=VBAT, F/B=GND-fyll, cirkulär outline) → verifiera → Gerbers/STEP.
Speglar weapon-flödet men cirkulärt. Körs efter receiver_place.py helmet."""
import subprocess, sys, math, shutil, os, pcbnew
PCB = "hardware/helmet-halo.kicad_pcb"; DSN = "hardware/helmet-halo.dsn"; SES = "hardware/helmet-halo.ses"
...
def unrouted(path):
    ...
    # ignorera 1-nods-nät (avsiktligt oanslutna modul-reservstift: XIAO 5V, reserv-GPIO, PPS, GAIN)
...
def finish(path):
    """Kopparplan: In1=GND, In2=VBAT, F/B=GND-fyll, cirkulär outline (r=49,5; 64-hörn)."""
    ...
    def add_zone(layer, net):
        ...
        for i in range(64):
            a = 2*math.pi*i/64; ch.Append(V(49.5*math.cos(a), 49.5*math.sin(a)))
...
print("REN board.")
os.system("rm -rf /tmp/gbh && mkdir -p /tmp/gbh")
subprocess.run(["kicad-cli", "pcb", "export", "gerbers", "-o", "/tmp/gbh/", PCB], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
subprocess.run(["kicad-cli", "pcb", "export", "drill", "-o", "/tmp/gbh/", PCB], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
subprocess.run(["bash", "-c", "cd /tmp/gbh && zip -q -r - . > /home/user/Strilas/hardware/helmet-halo-gerbers.zip"])
subprocess.run(["kicad-cli", "pcb", "export", "step", "-f", "--subst-models", "-o", "hardware/helmet-halo.step", PCB],
               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
