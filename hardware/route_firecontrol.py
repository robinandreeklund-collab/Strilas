#!/usr/bin/env python3
"""STRILAS — route fire-control-kortet (71×21, 2-lager, stack ovanpå P4 edge A).
...
PCB = "hardware/firecontrol.kicad_pcb"; DSN = "hardware/firecontrol.dsn"; SES = "hardware/firecontrol.ses"
...
def finish(path):
    ...
    def add_zone(layer):
        ...
        for x, y in [(-35.0, -10.3), (35.0, -10.3), (35.0, 10.3), (-35.0, 10.3)]: ch.Append(V(x, y))
...
_d = _d.replace(" GND ", " ")
open(DSN, "w").write(_d)
shutil.copy(PCB, "/tmp/_fc_placed.kicad_pcb")
...
for seed in range(1, 13):
    ...
    shutil.copy("/tmp/_fc_placed.kicad_pcb", PCB)
...
print("REN board.")
os.system("rm -rf /tmp/gbfc && mkdir -p /tmp/gbfc")
subprocess.run(["python3", "hardware/strip_fab_silk.py", PCB])  # dölj ref-des/titel på silk (NextPCB monterar från centroid/BOM)
subprocess.run(["kicad-cli", "pcb", "export", "gerbers", "-o", "/tmp/gbfc/", PCB], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
subprocess.run(["kicad-cli", "pcb", "export", "drill", "-o", "/tmp/gbfc/", PCB], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
subprocess.run(["bash", "-c", "cd /tmp/gbfc && zip -q -r - . > /home/user/Strilas/hardware/firecontrol-gerbers.zip"])
subprocess.run(["kicad-cli", "pcb", "export", "step", "-f", "--subst-models", "-o", "hardware/firecontrol.step", PCB],
               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
